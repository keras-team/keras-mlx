...
class CustomGradientTest(testing.TestCase):
    def test_gradient_comes_from_the_supplied_function(self):
        # The naive derivative of log(1 + exp(x)) overflows at 100 and gives
        # nan. The supplied gradient is the point of the decorator, so it must
        ...
        self.assertEqual(supplied.item(), 1.0)
        self.assertTrue(np.isnan(naive.item()))

    def test_gradient_with_several_arguments(self):
        @ops.custom_gradient
        def scaled_product(a, b):
            def grad(upstream):
                return (
                    ops.multiply(upstream, 10.0),
                    ops.multiply(upstream, 100.0),
                )
        ...
        self.assertEqual(da.item(), 10.0)
        self.assertEqual(db.item(), 100.0)

    def test_variable_arguments_are_unwrapped(self):
        log1pexp = _log1pexp()
        ...
        layer = Log1PExpLayer()
        # The layer adds its inputs, so the derivative with respect to them
        # is 1.
        grad = mx.grad(layer)(mx.array(100.0))

        self.assertEqual(grad.item(), 1.0)

    def test_forward_pass_returns_the_first_output(self):
        @ops.custom_gradient
        def doubled(x):
            def grad(upstream):
        ...
        self.assertAllClose(out, [2.0, 4.0, 6.0])
